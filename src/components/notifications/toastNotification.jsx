...
import { ErrorSvg, SuccessSvg } from "../../svgs";

export const successNotification = (toastObj) => {
    toast.custom(
        (t) => (
            <div key={t.id} className="toast_bar bg-[#22C55E]">
                <SuccessSvg onClick={() => toast.dismiss(t.id)} className="cursor-pointer" />
    ...
    toast.custom(
        (t) => (
            <div key={t.id} className="toast_bar bg-[#EE1D52]">
                <ErrorSvg onClick={() => toast.dismiss(t.id)} className="cursor-pointer" />
